...
9. [차량 간 거리 조절杆 → 속도 기어 매핑](#9-차량-간-거리-조절杆--속도-기어-매핑)
...
|----------------|------|-----------|
| **ETH** | 고대역폭 CAN-FD 백본 (진짜 이더넷 아님) | OBD-II 포트, X179 커넥터 |
...
본 프로젝트는 OBD-II / X179를 통해 **ETH** 버스에 접속합니다. FSD 관련 모든 제어 프레임은 이 버스에서 처리됩니다.
...
| **880** | **0x370** | `EPAS3P_sysStatus` | Nag 억제 목표 프레임 (카운터+1 에코 방법) | **CH** | 8 | — |
| **787** | **0x313** | `UI_trackModeSettings` | 트랙 모드 요청, 10개 신호 | ETH | 8 | — |
| **130** | **0x082** | `UI_tripPlanning` | 배터리 예열/예정 트리거 | ETH | 8 | 500ms |
...
펌웨어 추출 결과 이 프레임은 **74개 신호**를 포함하고 있습니다.
...
> **注意**：tuncasoftbildik 的实现用的是 `<< 5`（bit 61-63），我们和 flipper 项目用的是 `<< 4`（bit 60-62）。差一位，需要经验验证。```markdown
코드 예제 (C++)：
``````cpp
frame.data[7] &= ~(0x07 << 4);
...
#### HW4 Mux 2 중의 기타 알려진 신호 (펌웨어 추출에서)
...
레거시(HW1/HW2)는 CAN ID 1006을 1021 대신 사용하며, 프레임 구조는 유사합니다:
...
### 6.2 UI_smartSetSpeedOffset — 0x3FD mux 0 의 속도 오프셋

이것은 0x3FD 프레임 mux 0 의 속도 오프셋 신호로, 현재 HW3Handler 가 사용하고 있습니다.
...
3.2 절을 참조하십시오. 비트 19 = 0으로 설정 (HW4 추가 설정 비트 47 = 1).

### 방법 2: CAN 880 (0x370) 카운터 +1 에코

**버스: CH (섀시 CAN), 다른 물리적 접속 지점 필요.**

원리: EPAS3P_sysStatus 프레임을 모니터링하고, `handsOnLevel == 0` (손이 감지되지 않음, 즉 nag이 발생할 예정)일 때 가짜 프레임을 구성합니다:
1. 원래 프레임 복사
...
**당 in_progress != 0 일 때, 모든 CAN TX를 일시 중지해야 합니다**. OTA 업데이트에 방해가 되지 않도록 합니다.
...
**신호 이름과 열거형 값만 있으며, 비트 위치는 포함되어 있지 않습니다.** 비트 위치는 경험적 리버스 엔지니어링(출처 표기)에서 가져왔습니다.
...
| **mikegapinski/tesla-can-explorer** | JSON | 모든 모델, 577 프레임 40484 신호 (bit 위치 없음) | https://github.com/mikegapinski/tesla-can-explorer |
| **talas9/tesla_can_signals** | JSON | 모든 모델, bit 위치 있음 (하지만 UI/DAS 프레임 없음) | https://github.com/talas9/tesla_can_signals |
...
| 트랙 모드 (0x313) | HW3 | 완료 |
| Nag 킬러 (0x370 echo) | NagHandler | 완료 (컴파일 타임) |
...
| **속도 제한 인식 리스너** (DAS_fusedSpeedLimit) | CAN 0x399가 HW4 필터에 이미 포함되어 있으며, 읽기 전용 해석 추가 가능 | 낮음 |
| **사용자 속도 오프셋 리스너** (UI_userSpeedOffset) | CAN 0x3D9 필터에 추가 필요 | 낮음 |
| **ACC 속도 상한 리스너** (DAS_accSpeedLimit) | CAN 0x389 필터에 추가 필요 | 낮음 |
| **DAS 목표 속도 리스너** (DAS_setSpeed) | CAN 0x2B9 필터에 추가 필요 | 낮음 |
| **조향 보조 제어** (GTW_epasTuneRequest) | CAN 0x101이 CH 버스에 있으며, 다른 물리적 접속 필요 | 중간 |

---

## 20. 체크섬 계산 방법

Tesla는 통합된 체크섬 알고리즘을 사용합니다 (0x399, 0x370, 0x313 등의 프레임에 사용):
